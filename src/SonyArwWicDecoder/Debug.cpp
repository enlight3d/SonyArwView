...
// One-time init of the "enabled" flag and the log path + lock.
struct LogState {
    bool             enabled = false;
    ...
    LogState() noexcept {
        wchar_t buf[8] = {};
        const DWORD n = GetEnvironmentVariableW(L"SONY_ARW_WIC_DEBUG", buf, 8);
    ...
    ~LogState() noexcept {
        if (csInit) DeleteCriticalSection(&cs);
    }
};

LogState& State() noexcept {
    // Function-local static => thread-safe one-time init under MSVC.
    static LogState s;
    return s;
}
...
void Logf(const wchar_t* fmt, ...) noexcept {
    ...
    _snwprintf_s(line, _countof(line), _TRUNCATE,
                 ...
                 t.wMilliseconds, GetCurrentProcessId(), GetCurrentThreadId(),
                 body);
    WriteLine(line);
}
